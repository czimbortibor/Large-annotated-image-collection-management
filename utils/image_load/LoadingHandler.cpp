...
void LoadingHandler::loadImages_mt(QStringList* imageNames) {
	_loaderMT = std::unique_ptr<ImageLoaderMT, LoaderMT_deleter>
...
QList<GraphicsImage>* LoadingHandler::loadImages_st(QStringList* imageNames) {
	...
	_loaderST = std::unique_ptr<ImageLoaderST, LoaderST_deleter>(new ImageLoaderST(*imageNames, *results, size, *_imageCollection));
